...
class Wanted_listing extends Controller 
{
	...
	function __construct()
	{
		...
		$this->title='Wanted Listings';
		$this->main_content_mange='users/wanted_listing/manage';
		$this->main_content_add='users/wanted_listing/add';
		$this->main_content_edit='users/wanted_listing/edit';
		$this->add_title='Add Wanted Listings';
		$this->edit_title='Edit Wanted Listings';
		$this->controler_name='wanted_listing';
		$this->tablename='wanted_listing';
	...
	function index($row=1)
	{
		...
		$data['query']=$this->db->query("SELECT
			...
			wanted_listing.wanted_type,
			wanted_listing.wanted_terms,
			contacts.title,
			contacts.fname,
			contacts.lname
			FROM
			listings
			INNER JOIN wanted_listing ON listings.ref_id = wanted_listing.id
			INNER JOIN contacts ON wanted_listing.contact_info = contacts.id
			WHERE
			listings.type = 5 AND
			wanted_listing.listed_by = '".$this->session->userdata('user_id')."'");
