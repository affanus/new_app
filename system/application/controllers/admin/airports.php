...
class Airports extends Controller 
{
	...
	function __construct()
	{
		...
		$this->title='Airports';
		$this->main_content_mange='admin/manage/airports/mange';
		$this->main_content_add='admin/manage/airports/add';
		$this->main_content_edit='admin/manage/airports/edit';
		$this->add_title='Add Airport';
		$this->edit_title='Edit Airport';
		$this->controler_name='airports';
	...
	function index($row=1)
	{
		...
		$data['query']=$this->db->query("SELECT id,title,city,country,iata,icao,isactive From ".$this->tablename);
	...
	function csvimport(){
        ...
        $result =   $this->csvreader->parse_file(realpath(APPPATH . '../../csv/airports.csv'));
		$c=1;
		foreach ($result as $row):
			...
			if($row['iata']!=''):
					$data = array(
						...
						'iata' => addslashes($row['iata']),
						'icao' => addslashes($row['icao']),
						'lat'=> $row['lat'],
						'long'=> $row['long']
					);
	...
	{   
		...
		$this->form_validation->set_rules('title', 'Airport name', 'trim|required');
		...
		$data = array(
			...
			'iata' => addslashes($this->input->post('iata')),
			'icao' => addslashes($this->input->post('icao')),
