...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	$row = $query->row(); 	
	?>
	<?= $title?><?= $row->title?></div></td>
    </tr>
</table>
<? 
$attributes = array('id' => 'form','name' => 'form');
echo form_open('admin/website_settings/edit_/'.$this->uri->segment(4), $attributes);?>
...
<? } ?>
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="35%"><span class="formtxt upcl">Title</span><br />
      
      <input type="text" name="title" id="title" class="txtfield" onkeypress="if (event.keyCode == 13) {document.form.submit();}" value="<?= $row->title?>" /></td>
  </tr>
  <tr>
    <td style="padding-top:10px;"><span class="formtxt upcl">Email</span><br />
      <input type="text" name="email" id="email" class="txtfield" onkeypress="if (event.keyCode == 13) {document.form.submit();}" value="<?= $row->value?>"/></td>
  </tr>
  <tr>
    <td style="padding-top:10px;"><span class="formtxt upcl">Isactive</span><br />
      <select name="isactive" id="isactive" class="selmenu" style="width:210px;">
        <option value="1" <? if($row->status == '1') { ?>selected="selected"<? } ?>>Active</option>
        <option value="0" <? if($row->status == '0') { ?>selected="selected"<? } ?>>Inactive</option>
        </select></td>
  </tr>
  <tr>
