							...
							<div class="col-lg-6">
                        		<div class="form-group">
                            		<select class="form-control select2-list"  required id="man_id" name="man_id">
                                    	...
                                    	<option value="<?=$row_man->id?>" <? if($row_man->id==$row->man_id):?> selected="selected"<? endif;?>><?= stripslashes($row_man->title);?></option>
                                        	<? endforeach; ?>
                                        <? endif; ?>
														
									</select>
									<label for="man_id">Engine Manufacturer </label>
                                </div>
                            </div>    
							<div class="col-lg-6">
								<div class="form-group">
									<input type="text" class="form-control" name="title" id="title" required value="<?=stripslashes($row->title)?>">
									<label for="title">Engine Type</label>
								</div>
							</div>
						</div>
