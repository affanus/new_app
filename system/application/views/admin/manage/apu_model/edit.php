						...
						<div class="row">
							<div class="col-lg-6">
                        		<div class="form-group">
                            		<select class="form-control select2-list"  required id="type_id" name="type_id">
                                    	<option value=""></option>
                                    	<?   if($query_aircraft_man->num_rows() != 0) : 
											foreach($query_aircraft_man->result() as $row_man):?>
                                    	<option value="<?=$row_man->id?>" <? if($row_man->id==$row->type_id):?> selected="selected"<? endif;?>><?= stripslashes($row_man->man_title);?> <?= stripslashes($row_man->title);?></option>
                                        	<? endforeach; ?>
                                        <? endif; ?>
														
									</select>
									<label for="man_id">Aircraft Type </label>
                                </div>
                            </div>    
							<div class="col-lg-6">
								<div class="form-group">
									<input type="text" class="form-control" name="title" id="title" required value="<?=stripslashes($row->title)?>">
									<label for="title">Aircraft Model</label>
								</div>
							</div>
						</div>
