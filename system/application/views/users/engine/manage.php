   ...
   $c = 1;
	 foreach($query->result() as $row): ?>
								<tr>
                                    ...
                                    <td><?= $row->esn;?></td>
                                    <td><?= $row->e_man_title;?> <?= $row->e_type_title;?> <?= $row->e_model_title;?></td>
									<td><?= $row->title;?> <?= $row->fname;?> <?= $row->lname;?></td>
									<td class="text-right">
										<a href="<?php echo base_url();?>user/<?=$controler_name;?>/edit/<?= $row->id;?>" class="btn btn-icon-toggle" data-toggle="tooltip" data-placement="top" data-original-title="Edit <?= $row->e_man_title;?> <?= $row->e_type_title;?> <?= $row->e_model_title;?>"><i class="fa fa-pencil"></i></a>
                                        <? if($row->isactive == '1') { ?>
											<a href="<?php echo base_url();?>user/<?=$controler_name;?>/update_status/<?= $row->id;?>/0" class="btn btn-icon-toggle style-success " data-toggle="tooltip" data-placement="top" data-original-title="Status: Published" onclick="return confirm('Are You Sure To Un Publish <?= $row->e_man_title;?> <?= $row->e_type_title;?> <?= $row->e_model_title;?>?')"><i class="md md-cloud-done"></i></a>
                                        <? }else{?>
                                        	<a href="<?php echo base_url();?>user/<?=$controler_name;?>/update_status/<?= $row->id;?>/1" class="btn btn-icon-toggle style-danger " data-toggle="tooltip" data-placement="top" data-original-title="Status: Un Published" onclick="return confirm('Are You Sure To Publish <?= $row->e_man_title;?> <?= $row->e_type_title;?> <?= $row->e_model_title;?>?')"><i class="md md-cloud-off"></i></a>
                                        <? }?>
										<a href="<?php echo base_url();?>user/<?=$controler_name;?>/del/<?= $row->id;?>"  class="btn btn-icon-toggle " data-toggle="tooltip" data-placement="top" data-original-title="Delete <?= $row->e_man_title;?> <?= $row->e_type_title;?> <?= $row->e_model_title;?>" onclick="return confirm('Are You Sure To Delete <?= $row->e_man_title;?> <?= $row->e_type_title;?> <?= $row->e_model_title;?>?')"><i class="fa fa-trash-o"></i></a>
									</td>
								</tr>
  <? 
  $c++;
